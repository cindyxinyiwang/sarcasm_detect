...
class SVM(object):
	def __init__(self, pos_file, neg_file):
		...
		#self.neg_data = np.asarray()
		#self.data = np.concatenate((self.pos_data, self.neg_data))
	
		self.x_train = [[1, 2, 0], [1, 2, 3], [2, 2, 1], [0, 1, 5], [0, 0, 7], [-1, 2, 6]]
		self.y_train = [1, 1, 1, 0, 0, 0]
	...
	def test(self, file_name, class_id):
		...
		#y_predict = self.pipeline.predict([[0, -1, 8], [1, 2, 0]])
		y_predict = self.svc.predict([[0, -1, 8], [1, 2, 0]])
		print (y_predict)
...
if __name__ == "__main__":
	...
	#model.train(0.09)
	#model.get_stat()
	model = SVM("postrain.npy", "neg_data/negaa.npy")
	model.train()
